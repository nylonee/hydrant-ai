...
object Settings {
  ...
  val FILE_POINTING_TO_THE_IMAGES_ROOT_DIRECTORY: File = new File("images")
  ...
  def getNetworkConfiguration: MultiLayerConfiguration = {
    new NeuralNetConfiguration.Builder()
      ...
      .layer(5, new OutputLayer.Builder(LossFunctions.LossFunction.MCXENT) // to be used with Softmax
        .nOut(numClasses)
        .activation(Activation.SOFTMAX) // because I have two classes: hydrant or no-hydrant
        .build)
  ...
  def predictLabels(model: MultiLayerNetwork, iterator: RecordReaderDataSetIterator): Unit = {
    ...
    while (iterator.hasNext && counter < 20) {
      ...
      println("For a single example that is labeled [" + expectedResult + "] the model predicted [" + modelPrediction + "]")
      counter = counter + 1
    }
  }
}
